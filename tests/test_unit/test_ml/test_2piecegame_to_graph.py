...
def two_piece_game():
    ...
    grid = {
        ...
        (1, 1): (Piece(BLACK, pieces.QUEEN, 1),),  # Place BLACK queen adjacent to both pieces so beetle isn't pinned
    }
...
def test_both_pieces_are_present(two_piece_game):
    ...
    assert graph.nodes_by_location[((1, 1), 0)].piece == Piece(BLACK, pieces.QUEEN, 1)
    ...
    assert Piece(BLACK, pieces.QUEEN, 1) in node_pieces

def test_adjacent_edge_between_two_pieces(two_piece_game):
    game, graph = two_piece_game
...
def test_one_empty_space_above_queen(two_piece_game):
    ...
    # Now that BLACK has a queen placed, the beetle can move on top of the WHITE queen
