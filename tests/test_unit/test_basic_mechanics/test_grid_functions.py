...
def test_can_identify_all_3_positions_around_a_white_piece_plus_a_black_piece():
    grid = {(0, 0):  [Piece(WHITE, 'ANT', 1)],
            (1, -1):  [Piece(BLACK, 'ANT', 1)]}
    locations = get_placeable_locations(grid, WHITE)
    assert len(locations) == 3

def test_can_remove_piece_next_to_1_other_piece():
    grid = {(0, 0): [Piece(WHITE, 'ANT', 1)],
            (2, 0): [Piece(WHITE, 'ANT', 2)],
            (1, -1): [Piece(BLACK, 'ANT', 2)]}
...
def test_cant_remove_piece_in_line():
    grid = {(0, 0): [Piece(WHITE, 'ANT', 1)],
            (2, 0): [Piece(WHITE, 'ANT', 2)],
            (4, 9): [Piece(BLACK, 'ANT', 2)]}
    assert can_remove_piece(grid, (2, 0)) == False

def test_cant_remove_piece_if_disconnects_hive():
    grid = {(6, 2): [Piece(WHITE, PieceName.ANT, 1)],
            (5, 1): [Piece(WHITE, PieceName.ANT, 2)],
            (4, 2): [Piece(WHITE, PieceName.ANT, 3)],
            (3, 1): [Piece(WHITE, PieceName.ANT, 4)],
            (8, 2): [Piece(WHITE, PieceName.ANT, 5)],
            (10, 2): [Piece(WHITE, PieceName.ANT, 6)]}
...
def test_piece_is_connected_to_all_others():
    grid = {(0, 0): [Piece(WHITE, 'ANT', 1)],
            (1, -1): [Piece(WHITE, 'ANT', 2)],
            (1, 1): [Piece(BLACK, 'ANT', 2)],
            (0, 2): [Piece(BLACK, 'ANT', 2)]}
